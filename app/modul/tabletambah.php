<!-- Wrapper Tabel -->
<div class="space-y-6">

  <!-- 1. Tabel Daftar Buku -->
  <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
    <h2 class="flex items-center gap-2 text-lg font-bold text-slate-800 p-6 pb-4">
      <i class="fa-solid fa-chart-simple text-[#524bee]"></i> Daftar Buku Perpustakaan
    </h2>
    <div class="overflow-x-auto">
      <table class="w-full text-left text-sm">
        <thead class="bg-slate-50 border-y border-slate-200 text-[11px] font-bold text-slate-600 uppercase">
          <tr>
            <th class="px-6 py-3.5">NO</th>
            <th class="px-6 py-3.5">KODE ISBN</th>
            <th class="px-6 py-3.5">JUDUL BUKU</th>
            <th class="px-6 py-3.5">KATEGORI</th>
            <th class="px-6 py-3.5">STOK</th>
            <th class="px-6 py-3.5">AKSI</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100">
          <?php
          $queryBuku = "SELECT b.*, k.kategori 
                        FROM tb_buku b 
                        LEFT JOIN tb_kategori k ON b.id_kategori = k.id_kategori 
                        ORDER BY b.id DESC";
          $stmtBuku = $db->query($queryBuku);
          $no = 1;

          if ($stmtBuku && $stmtBuku->rowCount() > 0):
            while ($buku = $stmtBuku->fetch(PDO::FETCH_ASSOC)):
          ?>
            <tr>
              <td class="px-6 py-4"><?= $no++ ?></td>
              <td class="px-6 py-4 text-slate-500"><?= htmlspecialchars($buku['isbn']) ?></td>
              <td class="px-6 py-4 font-bold"><?= htmlspecialchars($buku['judul']) ?></td>
              <td class="px-6 py-4">
                <span class="bg-slate-100 px-3 py-1 rounded-full text-xs">
                  <?= htmlspecialchars($buku['kategori'] ?? 'Umum') ?>
                </span>
              </td>
              <td class="px-6 py-4 font-semibold text-emerald-600"><?= htmlspecialchars($buku['stok_tersedia']) ?> unit</td>
              <td class="px-6 py-4">
                <a href="?delete_id=<?= $buku['id'] ?>" onclick="return confirm('Yakin ingin menghapus buku ini?')" class="text-rose-500 hover:text-rose-700">
                  <i class="fa-regular fa-trash-can"></i>
                </a>
              </td>
            </tr>
          <?php 
            endwhile;
          else: 
          ?>
            <tr>
              <td colspan="6" class="px-6 py-4 text-center text-slate-400">Belum ada data buku.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
  </div>
